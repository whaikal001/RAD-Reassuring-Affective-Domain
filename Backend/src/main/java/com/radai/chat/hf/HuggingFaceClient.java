...
public class HuggingFaceClient {
    ...
    * Parse HuggingFace API response - chat completions format
    ...
    private String cleanResponse(String text, String userInput) {
        ...
        // Remove obvious analysis/plan markers that models sometimes emit (case-insensitive, dot matches newline)
        text = text.replaceAll("(?is)(analysis:|assistant analysis:|thoughts:|plan:).*?(\\.|$)", " ");

        // Remove sentences that start with 'We need to', 'We are asked to', 'Wait,' etc.
        text = text.replaceAll("(?is)\\bwe (need to|are asked to|should)\\b.*?(\\.|$)", " ");
        text = text.replaceAll("(?is)wait[,\\s].*?(\\.|$)", " ");

        // If the model emitted stepwise reasoning like 'Then the user says:' keep only the last user-facing fragment
        int idx = text.lastIndexOf("Then the user says:");
        if (idx != -1) {
            text = text.substring(idx + "Then the user says:".length()).trim();
        }

        // Also strip common assistant labels and keep the trailing answer if present
        String[] markers = new String[]{"Assistant reply:", "Assistant:", "Response:", "Answer:"};
        for (String m : markers) {
            int i = text.lastIndexOf(m);
            if (i != -1) {
                text = text.substring(i + m.length()).trim();
                break;
            }
        }

        // Remove any leftover bracketed content and collapse whitespace
        text = text.replaceAll("\\[.*?\\]", " ");
